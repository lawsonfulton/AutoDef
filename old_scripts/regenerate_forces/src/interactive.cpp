...
int main(int argc, char **argv) {
    ...
    fs::path sim_params_path = training_data_root / "parameters.json";

    //Remove everything in oputput dir
    ...
    // Load displacements
    std::cout << "Finding recorded displacements..." << std::endl;
    std::vector<MatrixXd> recorded_displacements;
    std::vector<int> displacement_nums;
    for (auto i = fs::directory_iterator(training_data_root); i != fs::directory_iterator(); i++)
    {
        fs::path cand_path = i->path();

        std::string name = cand_path.filename().string(); 
        std::string substr = "displacements";
        if(strncmp(name.c_str(), substr.c_str(), substr.size()) == 0) { // Is displacement
            std::string num_str = name.substr(substr.size()+1);
            displacement_nums.push_back(std::stoi(num_str, nullptr));
        }
    }
    std::cout << "Loading recorded displacements..." << std::endl;
    std::sort(displacement_nums.begin(), displacement_nums.end());
    for (std::vector<int>::iterator i = displacement_nums.begin(); i != displacement_nums.end(); ++i)
    {   
        fs::path dis_path = training_data_root / ("displacements_" + std::to_string(*i) + ".dmat");
        MatrixXd U;
        igl::readDMAT(dis_path.string(), U);
        recorded_displacements.push_back(U);
    }
    ...
    for(auto element: tets->getImpl().getElements()) {
        element->setDensity(sim_params["density"]);//1000.0);
    ...
    {   
        // Update with saved displacements
        MatrixXd new_U = recorded_displacements[current_frame].transpose();
        auto q = mapDOFEigen(tets->getQ(), world);
        q = Eigen::Map<VectorXd>(new_U.data(), new_U.rows() * new_U.cols()); 
