...
def main():
    ...
    if not options.skip_train:
        ...
        components = None
        files = None
        for inner_dim in range(2, 16):
            for outer_dim in range(6, 31):
                for i in range(1,4):
                    learning_config['learning_config']['autoencoder_config']['search_for_dims'] = False
                    learning_config['learning_config']['autoencoder_config']['fixed_inner_dim'] = inner_dim
                    learning_config['learning_config']['autoencoder_config']['fixed_outer_dim'] = outer_dim
                    training_results, mem, components, files = build_model(learning_config, model_path, force=True, mem=mem, components=components, files=files, do_pca_compare = False)
                    with open('figure data/error_heatmap/X/training_results_%d_%d_%d.json' % (inner_dim, outer_dim, i), 'w') as f:
    ...
    learning_config = {
        ...
        'learning_config': {
            ...
            'autoencoder_config': {
                ...
                "search_for_dims": training_params['search_for_dims'], # Otherwise just use fixed inner and outer dims
                "fixed_inner_dim": training_params.get('fixed_inner_dim', -1),
                "fixed_outer_dim": training_params.get('fixed_outer_dim', -1),
